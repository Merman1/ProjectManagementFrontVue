      ...
      <div class="h-screen bg-gray-50 flex justify-center items-center ">
        <div class="border border-gray-300 rounded-md p-[20px] w-[80vw] h-[80vh]  flex flex-row">
          <div class="border border-gray-300 rounded-md p-[30px] w-[30%] ">
            <h3 class="text-xl font-semibold mb-4">Wszystkie zgłoszenia</h3>
            <div class="overflow-y-auto" style="max-height: 60vh;">
              <div v-for="(task, index) in tasks" :key="index"
                class="bg-white hidden:bg-gray-300 p-4 rounded-md shadow-md cursor-pointer mb-4"
                @click="selectTask(task)">
                <h3 class="text-lg bg-cyan-700 text-white font-semibold">{{ task.scrum }}</h3>
                <div class="bg-white">{{ task.nazwa }}</div>
                <div class="bg-white">{{ task.etap }}</div>
              </div>
            </div>
          </div>
          <div class=" w-[60%] ml-[20px] flex-1 overflow-auto">
            <div class="mb-4 text-left">
              ...
              <div class='my-10' v-if="selectedTask">
                <div class="space-y-2">
                  <h2 class="text-2xl font-semibold text-left">{{ selectedTask?.scrum }}</h2>
                  <p class="text-left">{{ selectedTask?.nazwa }}</p>
                  <p class="text-left">{{ selectedTask?.etap }}</p>
                </div>
                <hr class="my-4 border-dashed border-gray-300">
                <div class="mt-8 overflow-y-auto">
                  ...
                  <div v-else>
                    <div v-for="comment in comments" :key="comment.id">
                      ...
                      <p>Treść: {{ comment.opis }}</p>
                      <p>Data dodania: {{ comment.dataDodania }}</p>
                      <hr class="my-4 border-dashed border-gray-300">
                    </div>
                  </div>
                  <textarea v-model="newComment" class="mt-4 border border-gray-300 rounded-md p-2 w-full"
                    placeholder="Dodaj nowy komentarz"></textarea>
                  <button @click="addComment" class="mt-2 px-4 py-2 bg-cyan-700 text-white rounded-md">Dodaj
                    komentarz</button>
                </div>
              </div>
            </div>
...
export default {
  ...
  methods: {
    ...
    addComment() {
      if (!this.userData) {
        console.error('Dane użytkownika nie są dostępne.');
        return;
      }
      if (this.selectedTask && this.newComment.trim()) {
        ...
        console.error('Nie wybrano zadania lub komentarz jest pusty.');
      }
    },
    ...
    selectTask(task) {
      ...
      this.newComment = '';
      this.fetchAllCommentsForIssue(task.id);
    },
